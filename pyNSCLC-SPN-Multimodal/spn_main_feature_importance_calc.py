...
data =  np.concatenate((data_ct_doctor, data_ct_follow,data_ct_biopsy), axis=0)
labels = np.concatenate((labels_doctor, labels_follow, labels_biopsy), axis=0)
INFO_TRAIN = pd.concat([INFO_DOCTOR,INFO_FOLLOW, INFO_BIOPSY],axis=0)
...
data =  np.concatenate((data_pet_doctor, data_pet_follow, data_pet_biopsy), axis=0)
labels = np.concatenate((labels_doctor, labels_follow, labels_biopsy), axis=0)
INFO_TRAIN = pd.concat([INFO_DOCTOR,INFO_FOLLOW, INFO_BIOPSY],axis=0)
...
items_no = [i for i in range (len(test_ct[:10]))]
