...
def shuffle_buckets(array, bucket_size):
    """
    Divides the array into len(array) // bucket_size buckets and shuffle the
    order of those buckets. The function is used to shuffle an array while
    keeping the relative order of each sequence of elements fixed
    :param array: np array to be bcuket-shuffled
    :param bucket_size: size of the bucket
    """
    i = 0
    buckets = []
    while i < array.shape[0]:
        buckets.append(array[i:i+bucket_size])
        i += bucket_size
    return np.random.permutation(buckets).flatten()
        ...
        while True:
            ...
            if self.batch_index == 0:
                ...
                if shuffle:
                    ...
                    index_array = shuffle_buckets(n, frames_per_step)
